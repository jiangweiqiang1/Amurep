...
def evaluate(model, dataSource, arg, POI_POI, epoch):
    ...
    for index in tqdm(range(len(test_pois_seq))):
        ...
        for i in range(len(poi_test)):
            ...
            pred, nextgeohashPred_2_test, nextgeohashPred_3_test, nextgeohashPred_4_test, nextgeohashPred_5_test, nextgeohashPred_6_test = model(
                input, 'test', arg, POI_POI, epoch, dataSource)
...
#                             subNodes2Index = [i - 1 for i in geoHashPOIs_last]
                            if isinstance(geoHashPOIs_last, int):
                                subNodes2Index = [geoHashPOIs_last - 1]
                            else:
                                subNodes2Index = [i - 1 for i in geoHashPOIs_last]
                        else:
    ...
    print()
    logging.info(str(result[1]) + ',' + str(result[5]) + ',' + str(result[10]) + ',' + str(result[20]) + ',' + str(
        np.average(totalMAP)))
    ...
    if arg['novelEval'] == True:  # for printing neatly.
        ...
        print()
        logging.info('')
        logging.info(dataSource + ' Finished!')
        logging.info('')
